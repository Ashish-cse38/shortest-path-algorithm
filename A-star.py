...
def find_distance(intersections, A, B):
  distance = math.sqrt((intersections[A][0] - intersections[B][0])**2 + (intersections[A][1] - intersections[B][1])**2)

  return distance


def shortest_path(intersections, roads, start, goal):
  if start == goal:
    return [5]
  ...
  heap = []
  path_cost = 0
  return path_finder(intersections, roads, start, goal, big_list, exists, heap, path_cost)

def path_finder(intersections, roads, start, goal, big_list, exists, heap, path_cost, node_le = None):
  exists.append(start)
  ...
  item_x = None
  if node_le is not None:
    for item in big_list:
      if start in item and node_le in item:
        item_x = item
        big_list.remove(item)
        break

  elif node_le is None:
    for item in big_list:
      if start in item:
        item_x = item
        big_list.remove(item)
  ...
  for node in roads[start]:
    if node not in exists:
      ucs_distance = find_distance(intersections, start, node) + path_cost

      heuristic_distance = find_distance(intersections, node, goal)

      A_distance = ucs_distance + heuristic_distance

      heapq.heappush(heap, (A_distance, node, ucs_distance, start))
      big_list.append(item_x + [node])


  dist, node_p, u_dist, n = heapq.heappop(heap)
  print(node_p)
  count = 1
  for item in heap:
    if node_p in item:
  ...
  return path_finder(intersections, roads, node_p, goal, big_list, exists, heap, u_dist, node_le)

print(shortest_path(intersections, roads, 8, 24))